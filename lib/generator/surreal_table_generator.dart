...
class SurrealTableGenerator extends GeneratorForAnnotation<SurrealTable> {
  ...
  String generateForAnnotatedElement(
    ...
    // Generate ORM extension methods (Task Group 4 & 5)
    ...
    // Generate Where Builder class (Task Group 9)
    buffer.write(_generateWhereBuilder(
      className: className,
      fields: fields,
    ));
  ...
  // ============================================================================
  // ORM Extension Generation (Task Group 4 & 5)
  // ============================================================================
  ...
  /// - tableName getter: Returns table name (Task Group 5)
  /// - tableStructure getter: Returns TableStructure (Task Group 5)
  ...
  String _generateOrmExtension({
    ...
    // Generate tableName static getter (Task Group 5)
    buffer.write(_generateTableNameGetter(tableName));
    buffer.writeln();

    // Generate tableStructure static getter (Task Group 5)
    buffer.write(_generateTableStructureGetter(className));
    buffer.writeln();
  ...
  /// Generates tableName static getter (Task Group 5).
  String _generateTableNameGetter(String tableName) {
    final buffer = StringBuffer();

    buffer.writeln('  /// Gets the table name for this entity.');
    buffer.writeln('  ///');
    buffer.writeln('  /// This is used by the ORM layer to determine which table');
    buffer.writeln('  /// to perform operations on.');
    buffer.writeln('  static String get tableName => \'$tableName\';');

    return buffer.toString();
  }

  /// Generates tableStructure static getter (Task Group 5).
  String _generateTableStructureGetter(String className) {
    final buffer = StringBuffer();
    final varName = '${_camelCase(className)}TableDefinition';

    buffer.writeln('  /// Gets the TableStructure for this entity.');
    buffer.writeln('  ///');
    buffer.writeln('  /// This is used by the ORM layer for schema validation');
    buffer.writeln('  /// and introspection.');
    buffer.writeln('  static TableStructure get tableStructure => $varName;');
  ...
  // ============================================================================
  // Where Builder Generation (Task Group 9)
  // ============================================================================

  /// Generates a where builder class for the entity.
  ///
  /// The where builder provides type-safe field accessors that return
  /// condition builders with operator methods.
  String _generateWhereBuilder({
    required String className,
    required List<_FieldInfo> fields,
  }) {
    final buffer = StringBuffer();
    final builderClassName = '${className}WhereBuilder';

    buffer.writeln('// Where Builder for $className');
    buffer.writeln('/// Type-safe where condition builder for $className entities.');
    buffer.writeln('///');
    buffer.writeln('/// This class provides field accessors that return condition builders');
    buffer.writeln('/// with type-safe operator methods.');
    buffer.writeln('///');
    buffer.writeln('/// Example:');
    buffer.writeln('/// ```dart');
    buffer.writeln('/// final users = await db.query<$className>()');
    buffer.writeln('///   .where((t) => t.fieldName.equals(value))');
    buffer.writeln('///   .execute();');
    buffer.writeln('/// ```');
    buffer.writeln('class $builderClassName {');
    buffer.writeln();

    // Generate field accessor properties
    for (final field in fields) {
      buffer.write(_generateFieldAccessor(className, field));
    }
  ...
  /// Generates a field accessor property for the where builder.
  String _generateFieldAccessor(String className, _FieldInfo field) {
    final buffer = StringBuffer();
    final fieldName = field.name;
    final dartTypeName = _getDartTypeName(field.dartType);

    // Determine the appropriate field condition type based on Dart type
    String fieldConditionType;
    if (dartTypeName == 'String') {
      fieldConditionType = 'StringFieldCondition';
    } else if (dartTypeName == 'int' || dartTypeName == 'double' || dartTypeName == 'num') {
      fieldConditionType = 'NumberFieldCondition<$dartTypeName>';
    } else if (dartTypeName == 'bool') {
      fieldConditionType = 'BoolFieldCondition';
    } else if (dartTypeName == 'DateTime') {
      fieldConditionType = 'DateTimeFieldCondition';
    } else if (field.nestedSchema != null) {
      // For nested objects, generate a nested condition builder
      return _generateNestedObjectAccessor(className, field);
    } else {
      // Default to StringFieldCondition for unknown types
      fieldConditionType = 'StringFieldCondition';
    }

    buffer.writeln('  /// Condition builder for the $fieldName field.');
    buffer.writeln('  $fieldConditionType get $fieldName => $fieldConditionType(\'$fieldName\');');
    buffer.writeln();

    return buffer.toString();
  }

  /// Generates a nested object accessor for complex field types.
  String _generateNestedObjectAccessor(String className, _FieldInfo field) {
    final buffer = StringBuffer();
    final fieldName = field.name;
    final nestedClassName = '${className}${_capitalize(fieldName)}Condition';

    buffer.writeln('  /// Nested condition builder for the $fieldName field.');
    buffer.writeln('  $nestedClassName get $fieldName => $nestedClassName(\'$fieldName\');');
    buffer.writeln();
  ...
  // ============================================================================
  // Query Builder Generation (Task Group 6 + Task Group 9)
  // ============================================================================
  ...
  String _generateQueryBuilder({
    ...
    buffer.writeln('///   .where((t) => t.field.equals(value))');
    ...
    buffer.writeln('  WhereCondition? _whereCondition;');
    buffer.writeln('  final Map<String, dynamic> _legacyWhereConditions = {};');
    buffer.writeln();
    buffer.writeln('  /// Private constructor - use Database.query<$className>() instead.');
    buffer.writeln('  $builderClassName._(this._db);');
    buffer.writeln();

    // Generate where method with builder callback (Task Group 9)
    buffer.write(_generateWhereMethod(builderClassName, className));
    buffer.writeln();
    ...
    // Generate legacy whereEquals methods for backward compatibility
    for (final field in fields) {
  ...
  /// Generates the where() method that accepts a where builder callback.
  String _generateWhereMethod(String builderClassName, String className) {
    final buffer = StringBuffer();

    buffer.writeln('  /// Adds a where condition using the type-safe where builder.');
    buffer.writeln('  ///');
    buffer.writeln('  /// Example:');
    buffer.writeln('  /// ```dart');
    buffer.writeln('  /// .where((t) => t.age.greaterThan(18) & t.status.equals(\'active\'))');
    buffer.writeln('  /// ```');
    buffer.writeln('  $builderClassName where(WhereCondition Function(${className}WhereBuilder) builder) {');
    buffer.writeln('    final whereBuilder = ${className}WhereBuilder();');
    buffer.writeln('    _whereCondition = builder(whereBuilder);');
  ...
  /// Generates a whereEquals method for a specific field (legacy support).
  ...
  String _generateExecuteMethod({
    ...
    buffer.writeln();
    buffer.writeln('    // Add type-safe where condition');
    buffer.writeln('    if (_whereCondition != null) {');
    buffer.writeln('      whereClauses.add(_whereCondition!.toSurrealQL(_db));');
    buffer.writeln('    }');
    buffer.writeln();
    buffer.writeln('    // Add legacy where conditions');
    buffer.writeln('    if (_legacyWhereConditions.isNotEmpty) {');
    buffer.writeln('      for (final entry in _legacyWhereConditions.entries) {');
    buffer.writeln('        final paramName = \'param_\${entry.key}\';');
    buffer.writeln('        await _db.set(paramName, entry.value);');
    buffer.writeln('        whereClauses.add(\'\${entry.key} = \\\$\$paramName\');');
    buffer.writeln('      }');
    buffer.writeln('    }');
    buffer.writeln();
    buffer.writeln('    if (whereClauses.isNotEmpty) {');
    buffer.writeln('      buffer.write(\' WHERE \');');
    buffer.writeln('      buffer.write(whereClauses.join(\' AND \'));');
    buffer.writeln('    }');
    buffer.writeln();
